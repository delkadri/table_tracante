...
try:
    while True:
        ...
        # Combiner les deux masques pour détecter le rouge
        mask_red = mask1 | mask2

        # Convertir l'image en niveau de gris pour détecter le chassis
        gray = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2GRAY)

        # Appliquer un seuillage pour détecter une zone grisâtre (le chassis)
        _, mask_chassis = cv2.threshold(gray, 50, 255, cv2.THRESH_BINARY)

        # Utiliser la zone du chassis comme masque de restriction
        mask_combined = cv2.bitwise_and(mask_red, mask_chassis)

        # Trouver les contours du masque combiné
